...
*/
const obj = { first: 'jack', last: 'chen' };
...
// if no argument is passed, default to empty object {}
function createStudent({ likesES2015 = true, likesJavaScript = true } = {}) {
  let student = 'The student';
  let js = 'JavaScript';
  let es = 'ES2015';

  if (likesES2015 && likesJavaScript) {
    return `${student} likes ${js} and ${es}!`;
  } else if (likesES2015 && !likesJavaScript) {
    return `${student} likes ${es}!`;
  } else if (!likesES2015 && likesJavaScript) {
    return `${student} likes ${js}!`;
  } else {
...
function reverseArray(arr) {
  ...
  // loop through arr, swap current number with number of same index counting backward
  for (let i = 0, n = arr.length / 2; i < n; i += 1) {
