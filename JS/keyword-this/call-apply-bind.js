...
function sumEvenArguments() {
  const evenArgs = Array.prototype.filter.call(arguments, argh => {
    return argh % 2 === 0;
  });

  return evenArgs.reduce((acc, next) => {
    return acc + next;
  });
}

function sumEvenArguments() {
  const arghs = Array.prototype.slice.call(arguments);

  return arghs.reduce((acc, next) => {
    if (next % 2 === 0) {
      return acc + next;
    }
...
function invokeMax(fn, num) {
  ...
  return function() {
    ...
    // otherwise, call fn and increment counter
    counter += 1;
...
function once(fn, thisArg) {
  // can be aclled only once
  let haveBeenCalled = false;

  // return function, set thisArg
  return function() {
    // when once() is called, returns the anonymous function; when anonymous function is called, return fn.apply().
    if (!haveBeenCalled) {
      haveBeenCalled = true;
...
function bind(fn, thisArg) {
  // get arguments passed with bind()
  const outerArgs = Array.prototype.slice.call(arguments, 2);

  return function() {
    // get arguments passed with anonymous function
...
function flip(fn, thisArg) {
  ...
  return function() {
    const innerArgs = Array.prototype.slice.call(arguments);
    // fn.length returns the # of parameters the function expects
    const allArgs = outerArgs.concat(innerArgs).slice(0, fn.length);
